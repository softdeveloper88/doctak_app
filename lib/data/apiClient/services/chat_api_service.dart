...
class ChatApiService {
  ...
  /// Delete a message
  Future<ApiResponse<Map<String, dynamic>>> deleteMessage({required String messageId}) async {
    try {
      final response = await networkUtils.handleResponse(await networkUtils.buildHttpResponse('/delete-message', method: networkUtils.HttpMethod.POST, request: {'ic': messageId}));
  ...
  Future<ApiResponse<Map<String, dynamic>>> updateReadStatus({required String userId, required String roomId}) async {
    try {
      final response = await networkUtils.handleResponse(await networkUtils.buildHttpResponse('/update-seen-status?user_id=$userId&room_id=$roomId', method: networkUtils.HttpMethod.POST));
      return ApiResponse.success(Map<String, dynamic>.from(response));
    } on ApiException catch (e) {
      return ApiResponse.error(e.message, statusCode: e.statusCode);
    } catch (e) {
  ...
  /// Send image message
  Future<ApiResponse<SendMessageModel>> sendImageMessage({required String userId, required String roomId, required String receiverId, required String imagePath, String? caption}) async {
    return sendMessageWithFile(userId: userId, roomId: roomId, receiverId: receiverId, message: caption ?? '', attachmentType: 'image', filePath: imagePath);
  }

  /// Send video message
  Future<ApiResponse<SendMessageModel>> sendVideoMessage({required String userId, required String roomId, required String receiverId, required String videoPath, String? caption}) async {
    return sendMessageWithFile(userId: userId, roomId: roomId, receiverId: receiverId, message: caption ?? '', attachmentType: 'video', filePath: videoPath);
  }

  /// Send document message
  Future<ApiResponse<SendMessageModel>> sendDocumentMessage({required String userId, required String roomId, required String receiverId, required String documentPath, String? caption}) async {
    return sendMessageWithFile(userId: userId, roomId: roomId, receiverId: receiverId, message: caption ?? '', attachmentType: 'document', filePath: documentPath);
  }

  /// Send audio message
  Future<ApiResponse<SendMessageModel>> sendAudioMessage({required String userId, required String roomId, required String receiverId, required String audioPath, String? caption}) async {
    return sendMessageWithFile(userId: userId, roomId: roomId, receiverId: receiverId, message: caption ?? '', attachmentType: 'audio', filePath: audioPath);
  }

  /// Get chat room info
  Future<ApiResponse<Map<String, dynamic>>> getRoomInfo({required String roomId}) async {
    try {
      // This endpoint might not exist in the original API
      // Adding it as a placeholder for room information
      final response = await networkUtils.handleResponse(await networkUtils.buildHttpResponse('/room-info?room_id=$roomId', method: networkUtils.HttpMethod.GET));
      return ApiResponse.success(Map<String, dynamic>.from(response));
    } on ApiException catch (e) {
      return ApiResponse.error(e.message, statusCode: e.statusCode);
    } catch (e) {
  ...
  Future<ApiResponse<Map<String, dynamic>>> markRoomAsRead({required String userId, required String roomId}) async {
    return updateReadStatus(userId: userId, roomId: roomId);
  }

  /// Get unread message count
  Future<ApiResponse<Map<String, dynamic>>> getUnreadCount({required String userId}) async {
    try {
      ...
      if (contactsResponse.success && contactsResponse.data != null) {
        ...
        return ApiResponse.success({'totalUnreadCount': totalUnread, 'unreadRooms': contacts.where((c) => (c.unreadCount ?? 0) > 0).length});
      } else {
  ...
  Future<ApiResponse<MessageModel>> getRoomMessenger({required String page, required String userId, required String roomId}) async {
  ...
  Future<ApiResponse<SendMessageModel>> sendMessageWithoutFile({
    ...
    return sendTextMessage(userId: userId, roomId: roomId, receiverId: receiverId, message: message);
  }
}
