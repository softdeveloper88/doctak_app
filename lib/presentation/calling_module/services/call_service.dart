...
class CallService extends ChangeNotifier {
  ...
  Future<void> _loadCallInfo() async {
    try {
      ...
      await prefs.initialize();
      //
      _currentCallId = await prefs.getString('active_call_id');
      _callerId = await prefs.getString('active_call_user_id');
      _callerName = await prefs.getString('active_call_name');
      _callerAvatar = await prefs.getString('active_call_avatar');
  ...
  Future<void> handleIncomingCall({
    ...
    try {
      ...
      //
      // DON'T navigate to call screen directly - wait for user to accept
      // The CallKit UI will handle showing the incoming call notification
      // Navigation to CallScreen happens in CallKitService when Event.actionCallAccept is received
  ...
  Future<Map<String, dynamic>> makeCall({
    ...
    if (!_isInitialized || _callApiService == null) {
      ...
      return {'success': false, 'message': 'Call service not initialized'};
    }
    //
