...
class CallProvider extends ChangeNotifier {
  ...
  Timer? _waitingForRemoteUserTimer;
  static const int _networkDisconnectTimeoutSeconds = 45; // Auto-disconnect after 45 seconds of no connection
  ...
  void _handleConnectionStateChanged(int state, int reason) {
    ...
    switch (state) {
      case 2: // Connecting
        ...
        _stopNetworkDisconnectTimer(); // Stop disconnect timer while connecting
        break;
      case 3: // Connected
        _updateCallState(connectionState: CallConnectionState.connected);
        _startConnectionHealthMonitoring();
        _resourceManager.setHighPerformanceMode(true);
        _stopNetworkDisconnectTimer(); // Connection restored - cancel disconnect timer
        print('✅ Connection restored - cancelled auto-disconnect timer');
        break;
      case 4: // Reconnecting
        ...
        _resourceManager.setHighPerformanceMode(false);
        _stopConnectionHealthMonitoring();
        // Start auto-disconnect timer (WhatsApp-like behavior)
        // Only if call was previously connected and remote user had joined
        if (_callState.isRemoteUserJoined) {
          _startNetworkDisconnectTimer();
        }
        break;
      case 5: // Failed
  ...
  // ============= AUTO-DISCONNECT TIMER METHODS (WhatsApp-like behavior) =============

  /// Start the network disconnect timer when connection is lost
  /// This gives the user visual feedback with countdown before auto-ending
  void _startNetworkDisconnectTimer() {
    _stopNetworkDisconnectTimer();
    if (!mounted) return;

    print('⏱️ Starting network disconnect timer ($_networkDisconnectTimeoutSeconds seconds)');

    int remainingSeconds = _networkDisconnectTimeoutSeconds;
    _updateCallState(reconnectCountdown: remainingSeconds);

    _networkDisconnectTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (!mounted) {
        timer.cancel();
        return;
      }
  ...
  /// Stop the network disconnect timer (e.g., when connection is restored)
  void _stopNetworkDisconnectTimer() {
    _networkDisconnectTimer?.cancel();
    _networkDisconnectTimer = null;
    if (mounted && _callState.reconnectCountdown > 0) {
      _updateCallState(reconnectCountdown: 0);
    }
  }

  /// Start timer to wait for remote user to join
  /// If remote user doesn't join within timeout, auto-end the call
  void _startWaitingForRemoteUserTimer() {
  ...
  void _stopWaitingForRemoteUserTimer() {
    _waitingForRemoteUserTimer?.cancel();
    _waitingForRemoteUserTimer = null;
  }

  /// Cancel all auto-disconnect timers
  void _stopAllAutoDisconnectTimers() {
    _stopNetworkDisconnectTimer();
    _stopWaitingForRemoteUserTimer();
  }
  ...
  void endCall() {
    ...
    _stopConnectionHealthMonitoring();
    _stopHeartbeat();
    _stopAllAutoDisconnectTimers(); // Stop auto-disconnect timers
    ...
    // Update call end reason if not already set
    if (_callState.callEndReason == CallEndReason.none) {
      _updateCallState(callEndReason: CallEndReason.userEnded);
    }
  ...
  void _handleUserJoined(int remoteUid, int elapsed) {
    ...
    _stopWaitingForRemoteUserTimer();
    _stopNetworkDisconnectTimer(); // Also stop any disconnect timer
    ...
    _updateCallState(
      ...
      isRemoteUserJoined: true,
      connectionState: CallConnectionState.connected,
      reconnectCountdown: 0, // Reset countdown
    );
  ...
  void _handleUserOffline(int remoteUid, int reason) {
    ...
    // Determine call end reason based on offline reason
  ...
  String _getDisconnectReason(int reason) {
    switch (reason) {
      case 0:
        return 'USER_OFFLINE_QUIT_DEPRECATED';
      case 1:
        return 'USER_OFFLINE_QUIT (Normal disconnect)';
      case 2:
        return 'USER_OFFLINE_DROPPED (Connection lost)';
      case 3:
