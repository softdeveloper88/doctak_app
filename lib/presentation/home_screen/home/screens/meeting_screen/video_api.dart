...
Future<ApiResponse> allowJoinMeet(context,meetingId,userId) async {
  try {
    ...
    Map<String ,dynamic> response= await networkUtils.handleResponse(await networkUtils.buildHttpResponse('/allow-join-request?userId=$userId&meetingId=$meetingId', method: networkUtils.HttpMethod.GET,));
...
Future<ApiResponse> rejectJoinMeet(context,meetingId,userId) async {
  try {
    ProgressDialogUtils.showProgressDialog();
    Map<String ,dynamic> response= await networkUtils.handleResponse(await networkUtils.buildHttpResponse('/reject-join-request', method: networkUtils.HttpMethod.GET,request: {'userId': userId,
      'meetingId':meetingId
    }));
...
Future<ApiResponse> sendMessage(meetingId,message,senderId) async {
  try {
    Map<String ,dynamic> response= await networkUtils.handleResponse(await networkUtils.buildHttpResponse('/send-message-meeting', method: networkUtils.HttpMethod.POST,request: {
      'meeting_id':meetingId,
      'message':message,
      'user_id':senderId
    }));
...
Future<ApiResponse> changeMeetingStatus(context,meetingId,userId,action,status) async {
  try {
    Map<String ,dynamic> response= await networkUtils.handleResponse(await networkUtils.buildHttpResponse('/meeting-update-status?meeting_id=$meetingId&user_id=$userId&action=$action&status=$status', method: networkUtils.HttpMethod.GET));
...
Future<ApiResponse> sendInviteMeeting(channel,userId) async {
  try {
    Map<String ,dynamic> response= await networkUtils.handleResponse(await networkUtils.buildHttpResponse('/send-meeting-invitation', method: networkUtils.HttpMethod.POST,request: {
      'channel':channel,
      'userId':userId
    }));
...
Future<ApiResponse> updateMeetingSetting({meetingId,startStopMeeting,addRemoveHost,shareScreen,raisedHand,sendReactions,toggleMicrophone,toggleVideo,enableWaitingRoom,requirePassword}) async {
  try {
    Map<String ,dynamic> response= await networkUtils.handleResponse(await networkUtils.buildHttpResponse('/meeting-settings/update', method: networkUtils.HttpMethod.POST,request: {
    'meeting_id' : meetingId,
...
Future<ApiResponse> setScheduleMeeting({title,date,time}) async {
  try {
    ...
    Map<String ,dynamic> response= await networkUtils.handleResponse(await networkUtils.buildHttpResponse('/save-schedule-meeting?title=$title&date=$date&time=$time', method: networkUtils.HttpMethod.GET));
    ProgressDialogUtils.hideProgressDialog();
