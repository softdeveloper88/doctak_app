...
class _AddEditWorkScreenState extends State<AddEditWorkScreen> with SingleTickerProviderStateMixin {
  late AnimationController _animationController;
  late Animation<double> _fadeAnimation;

  List<String> listWorkType = ['work', 'university', 'high_school'];
  List<String> privacyList = ['global', 'private', 'only me'];
  List<String> positionList = ['Resident Physician', 'Attending Physician', 'Fellow', 'Consultant', 'Surgeon', 'General Practitioner', 'Medical Officer', 'Researcher', 'Educator/Teacher/Professor'];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: DoctakAppBar(
        title: widget.updateWork != null ? translation(context).lbl_edit : translation(context).lbl_add_experience,
      ...
      body: FadeTransition(
        ...
        child: Padding(
          ...
          child: SingleChildScrollView(child: Column(children: [_buildWorkInfoFields(), const SizedBox(height: 20)])),
        ),
      ),
    );
  }
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Container(
                      ...
                      child: Icon(Icons.work_outline, color: Colors.blue[700], size: 20),
                    ),
                    const SizedBox(width: 12),
                    Text(
                      widget.updateWork != null ? translation(context).lbl_update_experience_details : translation(context).lbl_add_experience_details,
                ...
                // Date fields
                const SizedBox(height: 20),
                ProfileDateWidget(isEditModeMap: true, index: 2, label: translation(context).lbl_start_date, value: updateWork?.startDate ?? '', onSave: (value) => updateWork?.startDate = value),

                const SizedBox(height: 20),
                ProfileDateWidget(isEditModeMap: true, index: 2, label: translation(context).lbl_end_date, value: updateWork?.endDate ?? '', onSave: (value) => updateWork?.endDate = value),
                ...
                child: ElevatedButton(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      const SizedBox(width: 8),
                      Text(widget.updateWork != null ? translation(context).lbl_update : translation(context).lbl_add, style: const TextStyle(fontWeight: FontWeight.bold)),
                    ],
                  ),
                ),
              ),
  ...
  void _saveExperience() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(widget.updateWork != null ? translation(context).msg_experience_updated : translation(context).msg_experience_added, style: const TextStyle(color: Colors.white)),
