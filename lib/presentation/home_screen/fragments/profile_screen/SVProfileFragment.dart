          ...
          builder: (context, state) {
            if (state is PaginationLoadingState) {
              ...
              return SingleChildScrollView(
                child: Column(
                  children: [
                     SVProfileHeaderComponent(userProfile: profileBloc.userProfile,profileBoc:profileBloc),
                    16.height,
                    Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Text('${profileBloc.userProfile?.user?.firstName??''} ${profileBloc.userProfile?.user?.lastName??''}', style: boldTextStyle(size: 20)),
                        4.width,
                        Image.asset('images/socialv/icons/ic_TickSquare.png', height: 14, width: 14, fit: BoxFit.cover),
                      ],
                    ),
                    Text(profileBloc.userProfile?.user?.specialty??'Student', style: secondaryTextStyle(color: svGetBodyColor())),
                    24.height,
                    AppButton(
                      ...
                      children: [
                        Column(
                          children: [
                            ...
                            4.height,
                            Text('${profileBloc.userProfile?.totalPosts??''}', style: boldTextStyle(size: 18)),
                          ],
                        ),
                        Column(
                          children: [
                            ...
                            4.height,
                            Text(profileBloc.userProfile?.totalFollows?.totalFollowers??'', style: boldTextStyle(size: 18)),
                          ],
                        ),
                        Column(
                          children: [
                            ...
                            4.height,
                            Text(profileBloc.userProfile?.totalFollows?.totalFollowings??'', style: boldTextStyle(size: 18)),
                          ],
                        )
                      ],
                    ),
