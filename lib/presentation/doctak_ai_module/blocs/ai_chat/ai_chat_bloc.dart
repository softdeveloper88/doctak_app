...
class AiChatBloc extends Bloc<AiChatEvent, AiChatState> {
  ...
  Future<void> _sendMessageToAPI({
    ...
    try {
      ...
      if (!emit.isDone) {
        emit(MessageSendError(sessions: sessions, selectedSession: selectedSession, messages: updatedMessages, message: errorMessage));
      }
      // Exit early
      return;
    }
    ...
    try {
      ...
      // Check if emit is still valid before emitting
      if (!emit.isDone) {
        emit(MessageSendError(sessions: sessions, selectedSession: selectedSession, messages: updatedMessages, message: 'Failed to parse AI response. Please try again later.'));
      }
    ...
    if (isFirstMessage && event.suggestTitle) {
      try {
        final title = await aiChatApi.suggestTitle(sessionId, event.message, aiMessage.content);
  ...
  Future<void> _onLoadSessions(LoadSessions event, Emitter<AiChatState> emit) async {
    ...
    try {
      ...
      if (hasCache) {
        ...
        emit(SessionsLoadError(message: 'Failed to load sessions: ${e.toString()}'));
      }
    }
  }

  Future<void> _onCreateSession(CreateSession event, Emitter<AiChatState> emit) async {
    ...
    try {
      final newSession = await aiChatApi.createChatSession(name: event.name);
      ...
      emit(SessionCreated(sessions: updatedSessions, newSession: newSession));
      ...
      emit(SessionCreateError(message: 'Failed to create session: ${e.toString()}', sessions: currentSessions));
    }
  }

  Future<void> _onSelectSession(SelectSession event, Emitter<AiChatState> emit) async {
    ...
    if (selectedSession == null) {
      emit(SessionLoadError(message: 'Session not found. It may have been deleted or is unavailable.', sessions: currentSessions));
      return;
    }
  ...
  Future<void> _onSendMessage(SendMessage event, Emitter<AiChatState> emit) async {
    ...
    if (!(state is SessionSelected || state is MessageSent)) {
      ...
      if (state is! SessionSelected) {
        try {
          ...
          final userMessage = AiChatMessageModel(id: DateTime.now().millisecondsSinceEpoch, sessionId: sessionId, role: MessageRole.user, content: event.message, createdAt: DateTime.now());

          final updatedMessages = [userMessage];

          // Update state to show user message immediately
          emit(MessageSending(sessions: updatedSessions, selectedSession: newSession, messages: updatedMessages, isFirstMessage: true));
          ...
          await _sendMessageToAPI(
            ...
            event: event,
            currentState: MessageSending(sessions: updatedSessions, selectedSession: newSession, messages: updatedMessages, isFirstMessage: true),
    ...
    emit(MessageSending(sessions: currentSessions, selectedSession: selectedSession, messages: updatedMessages, isFirstMessage: currentMessages.isEmpty));
    ...
    try {
      await _sendMessageToAPI(
        sessionId: sessionId.toString(),
        event: event,
        currentState: MessageSending(sessions: currentSessions, selectedSession: selectedSession, messages: updatedMessages, isFirstMessage: currentMessages.isEmpty),
        userMessage: userMessage,
        emit: emit,
      );
  ...
  Future<void> _onDeleteSession(DeleteSession event, Emitter<AiChatState> emit) async {
    ...
    try {
      ...
      final updatedSessions = currentSessions.where((session) => session.id != event.sessionId).toList();

      // Update local storage
      await AiChatLocalStorage.saveSessions(updatedSessions);

      emit(SessionDeleted(sessions: updatedSessions, nextSession: response['nextSession']));

      // If there's a next session and we were viewing the deleted session,
      // automatically select the next session
      if (response['nextSession'] != null && state is SessionSelected && (state as SessionSelected).selectedSession.id == event.sessionId) {
        add(SelectSession(sessionId: response['nextSession'].id));
      }
    } catch (e) {
      emit(SessionDeleteError(message: 'Failed to delete session: ${e.toString()}', sessions: currentSessions));
    }
  }

  Future<void> _onRenameSession(RenameSession event, Emitter<AiChatState> emit) async {
    ...
    emit(SessionUpdating(sessions: currentSessions, selectedSession: currentSelectedSession, messages: currentMessages ?? []));

    try {
      debugPrint("⚠️⚠️⚠️ Making API call to rename session");
      final updatedSession = await aiChatApi.renameSession(event.sessionId, event.name);
      ...
      bool isSelectedSession = currentSelectedSession != null && currentSelectedSession.id.toString() == event.sessionId;
      ...
      if (isSelectedSession && currentMessages != null) {
        ...
        if (state is SessionSelected || state is SessionUpdating) {
          emit(SessionSelected(sessions: updatedSessions, selectedSession: updatedSession, messages: currentMessages));
        } else if (state is MessageSending) {
          emit(MessageSending(sessions: updatedSessions, selectedSession: updatedSession, messages: currentMessages, isFirstMessage: (state as MessageSending).isFirstMessage));
        } else if (state is MessageSent) {
          emit(
            MessageSent(
      ...
      emit(SessionUpdateError(message: 'Failed to rename session: ${e.toString()}', sessions: currentSessions));
    }
  }

  Future<void> _onSubmitFeedback(SubmitFeedback event, Emitter<AiChatState> emit) async {
    if (state is! SessionSelected) return;

    final currentState = state as SessionSelected;

    try {
      final updatedMessage = await aiChatApi.submitFeedback(event.messageId, event.feedback);
      ...
      // Update local storage
      await AiChatLocalStorage.saveMessages(currentState.selectedSession.id.toString(), updatedMessages);

      if (!emit.isDone) {
        emit(SessionSelected(sessions: currentState.sessions, selectedSession: currentState.selectedSession, messages: updatedMessages));
      }
    } catch (e) {
      if (emit.isDone) return;

      // Just show a temporary error, but don't change state
      emit(FeedbackError(sessions: currentState.sessions, selectedSession: currentState.selectedSession, messages: currentState.messages, message: 'Failed to submit feedback: ${e.toString()}'));
  ...
  void _onClearCurrentSession(ClearCurrentSession event, Emitter<AiChatState> emit) {
    if (state is SessionsLoaded) {
