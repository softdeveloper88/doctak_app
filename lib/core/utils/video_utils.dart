...
class VideoUtils {
  ...
  static bool isResolutionSupported(Size videoSize) {
    return videoSize.width <= MAX_SUPPORTED_WIDTH && videoSize.height <= MAX_SUPPORTED_HEIGHT;
  }
  ...
  static bool isHighResolution(Size videoSize) {
    return videoSize.width >= RESOLUTION_1440P.width || videoSize.height >= RESOLUTION_1440P.height;
  }
  ...
  static String getVideoErrorMessage(dynamic error) {
    ...
    if (errorStr.contains('exceeds_capabilities') || errorStr.contains('no_exceeds_capabilities')) {
      return 'Video quality too high for this device. Try a lower resolution video.';
    } else if (errorStr.contains('decoder_init_failed') || errorStr.contains('mediacodecrenderer')) {
  ...
  static bool isCodecError(dynamic error) {
    String errorStr = error.toString().toLowerCase();
    return errorStr.contains('exceeds_capabilities') || errorStr.contains('decoder') || errorStr.contains('mediacodec') || errorStr.contains('codec');
  }

  /// Get recommended video settings for device
  static Map<String, dynamic> getRecommendedVideoSettings() {
    return {'maxWidth': MAX_SUPPORTED_WIDTH, 'maxHeight': MAX_SUPPORTED_HEIGHT, 'preferredFormat': 'mp4', 'preferredCodec': 'h264'};
  }
