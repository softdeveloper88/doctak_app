    ...
    private fun cleanupCorruptedDatabases() {
        try {
            ...
            if (databasesDir != null && databasesDir.exists()) {
                ...
                val transportDatabases = databasesDir.listFiles { file ->
                ...
                transportDatabases?.forEach { dbFile ->
                    try {
                        ...
                        if (isDatabaseCorrupted(dbFile)) {
                            ...
                            android.util.Log.d("DoctakApplication", "Deleted potentially corrupted database: ${dbFile.name}")
                        }
                    } catch (e: Exception) {
                        android.util.Log.e("DoctakApplication", "Error checking database ${dbFile.name}", e)
                    }
                }
            }
    ...
    override fun attachBaseContext(base: Context) {
        ...
        Thread.setDefaultUncaughtExceptionHandler { thread, exception ->
            if (exception.message?.contains("database disk image is malformed") == true ||
                ...
                android.util.Log.e("DoctakApplication", "SQLite corruption detected, scheduling cleanup on next launch", exception)
